...
ENVIRON_NAME="Fragmenstein2"
conda create -n $ENVIRON_NAME -c rdkit python=3.7 pyrosetta tensorflow-gpu=1.13 rdkit joblib pandas qt requests docopt matplotlib cython nomkl --yes &&

eval "$(conda shell.bash hook)" && conda activate $ENVIRON_NAME &&
pip install rdkit_to_params planarity pickle5 && conda install -c conda-forge glew glm planarity openbabel --yes &&
